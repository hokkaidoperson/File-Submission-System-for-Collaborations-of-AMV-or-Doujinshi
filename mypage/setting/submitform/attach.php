...
echo_textbox([
    ...
    "prefill" => isset($_SESSION["submitformdata"][$number]["ext"]) ? hsc($_SESSION["submitformdata"][$number]["ext"]) : '',
...
echo_textbox([
    ...
    "width" => 5,
    "suffix" => "個",
    "prefill" => isset($_SESSION["submitformdata"][$number]["filenumber"]) ? hsc($_SESSION["submitformdata"][$number]["filenumber"]) : '',
...
echo_textbox([
    ...
    "width" => 5,
    "suffix" => "MB",
    "prefill" => isset($_SESSION["submitformdata"][$number]["size"]) ? hsc($_SESSION["submitformdata"][$number]["size"]) : '',
...
echo_textbox_mlt([
    [
        "prefill" => isset($_SESSION["submitformdata"][$number]["reso"][0]) ? hsc($_SESSION["submitformdata"][$number]["reso"][0]) : '',
    ...
    [
        "prefill" => isset($_SESSION["submitformdata"][$number]["reso"][1]) ? hsc($_SESSION["submitformdata"][$number]["reso"][1]) : '',
        "jspart" => 'onChange="validation_call(&quot;reso[1]&quot;);"',
        "width" => 5,
        "prefix" => "縦（高さ）",
        "suffix" => "px"
    ]
], "動画・画像ファイルの最大解像度（1以上の半角数字）", 'reso', 'reso', '※動画・画像ファイル以外の場合は解像度のチェックを行いません。<br>※入力が無い場合は、解像度の制限を設けません。', "number");
echo_textbox_mlt([
    [
        "prefill" => isset($_SESSION["submitformdata"][$number]["length"]) ? hsc((int)((int)$_SESSION["submitformdata"][$number]["length"] / 60)) : '',
        "jspart" => 'onChange="validation_call(&quot;length[0]&quot;);"',
        "width" => 5,
        "suffix" => "分"
    ],
    [
        "prefill" => isset($_SESSION["submitformdata"][$number]["length"]) ? hsc((int)$_SESSION["submitformdata"][$number]["length"] % 60) : '',
        "jspart" => 'onChange="validation_call(&quot;length[1]&quot;);"',
        "width" => 5,
        "suffix" => "秒"
    ]
], "動画・音声ファイルの最大再生時間（0分0秒以上の時間指定）", 'length', 'length', '※動画・音声ファイル以外の場合、再生時間は0分0秒として扱われます。<br>※複数個のファイルをこの入力欄に添付出来る設定にしている場合、この入力欄に添付するファイルの合計再生時間が、ここで指定する再生時間以下になっている必要があります。<br>※入力が無い場合は、再生時間の制限を設けません。', "number");
echo_check([
