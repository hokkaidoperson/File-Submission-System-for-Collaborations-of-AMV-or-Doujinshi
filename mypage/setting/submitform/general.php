...
], "動画・画像ファイルの最大解像度（1以上の半角数字）", 'reso', 'reso', '※現在はmp4ファイルのみ解像度のチェックに対応します。今後、対応するファイルの種類を拡充する予定です。<br>※入力が無い場合は、解像度の制限を設けません。<br>※外部URL経由の提出に関しては、チェックの対象外となります。', "number");
echo_textbox_mlt([
    [
...
], "動画の最大再生時間（0分0秒以上の時間指定）", 'length', 'length', '※現在はmp4ファイルのみ再生時間のチェックに対応します。今後、対応するファイルの種類を拡充する予定です。<br>※複数個のファイルをこの入力欄に添付出来る設定にしている場合、この入力欄に添付するファイルの合計再生時間が、ここで指定する再生時間以下になっている必要があります。<br>※入力が無い場合は、再生時間の制限を設けません。<br>※外部URL経由の提出に関しては、チェックの対象外となります。', "number");
echo_textbox([
...
], "提出可能な作品の総再生時間（0分0秒以上の時間指定）", 'worklength', 'worklength', '※再生時間の上限を設定出来ます。ユーザーが合計再生時間の上限を超えて作品を提出しようとすると、エラーをユーザーに表示します。<br>※入力が無い場合は、総再生時間の上限を設定しません。<br>※現在はmp4ファイルのみ再生時間のチェックに対応します。今後、対応するファイルの種類を拡充する予定です。非対応のファイルの再生時間は0分0秒として扱われます。<br>※外部URL経由の提出に関しては、チェックの対象外となります。', "number");
