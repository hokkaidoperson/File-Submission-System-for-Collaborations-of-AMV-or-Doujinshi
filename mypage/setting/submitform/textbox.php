...
if (!isset($_GET['number']) or !isset($_SESSION["submitformdata"][$number]["id"]) or ('textbox' != $_SESSION["submitformdata"][$number]["type"]))
...
foreach($disable_status as $key => $disable) {
    $form_setting[] = [
        "prefill" => isset($_SESSION["submitformdata"][$number]["max"][$key]) ? hsc($_SESSION["submitformdata"][$number]["max"][$key]) : '',
...
foreach($disable_status as $key => $disable) {
    $form_setting[] = [
        "prefill" => isset($_SESSION["submitformdata"][$number]["min"][$key]) ? hsc($_SESSION["submitformdata"][$number]["min"][$key]) : '',
...
foreach($disable_status as $key => $disable) {
    $form_setting[] = [
        "prefill" => isset($_SESSION["submitformdata"][$number]["width"][$key]) ? hsc($_SESSION["submitformdata"][$number]["width"][$key]) : '',
...
echo_textbox_mlt($form_setting, "入力欄の前に表示する文字（接頭辞）（50文字以内）", 'prefix', 'prefix', '※例えば、「https://www.nicovideo.jp/watch/sm」と指定すると、入力欄は、<br>　　https://www.nicovideo.jp/watch/sm[　　　　]<br>　のような見た目となります。');
$form_setting = [];
foreach($disable_status as $key => $disable) {
    $form_setting[] = [
...
echo_textbox_mlt($form_setting, "入力欄の後に表示する文字（接尾辞）（50文字以内）", 'suffix', 'suffix', '※例えば、「年」と指定すると、入力欄は、<br>　　[　　　　]年<br>　のような見た目となります。<br>※接頭辞と接尾辞を組み合わせる事も出来ます。例えば、接頭辞に「令和」、接尾辞に「年」と指定すると、<br>　　令和[　　　　]年<br>　のような見た目となります。');
echo_check([
...
let multiple_items = ["max", "min", "width", "prefix", "suffix"];
