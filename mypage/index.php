...
if (file_exists(DATAROOT . 'form/submit/done.txt')) {
    ...
    if ($general["from"] > time()) {
        ...
        if ($_SESSION["state"] == 'p') echo_alert('<p>ファイル提出受付は、' . $untildisp . 'に締め切りました。<br>
また、提出済みのファイル情報や、共通情報、ニックネームの編集機能も停止しました（メールアドレス、パスワードは編集可）。</p>
<p>「参加者・作品の一覧・編集」ページから、提出されたファイルや入力内容を一括ダウンロード出来ます。</p>
<p>ただし、主催者が認めれば、締め切り後の提出・編集が可能です（主催者自身の情報編集は常時可能です）。<br>
参加者からの要請を受諾して提出・編集機能を限定的に解禁する場合は、「権限コントロール」ページから設定して下さい。</p>', "danger", TRUE);
