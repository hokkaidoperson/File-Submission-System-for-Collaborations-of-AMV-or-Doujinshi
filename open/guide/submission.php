...
<p>ポータルサイトのサーバーに直接アップロード出来るファイルの最大サイズは <b><?php echo $maxsize; ?>MB</b> です（複数ファイルを提出する場合、それらの合計サイズが<?php echo $maxsize; ?>MB以下である必要があります）。<br>
アップロードしたいファイルのサイズが<?php echo $maxsize; ?>MB以下の場合は「<b>ファイルをサーバーに直接アップロードする</b>」を、<?php echo $maxsize; ?>MBを上回っている場合は「<b>外部のファイルアップロードサービスを利用して送信する</b>」を選択して下さい。</p>
...
<p>必要事項を入力し、<b>最後に必ず「送信する」を押下して下さい</b>（「送信する」を押下しないと送信されません。ファイルの提出を取りやめる場合は、そのまま提出画面を離れて下さい。）。</p>
