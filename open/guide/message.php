...
<p>メッセージの宛先を選択し、件名（省略可）とメッセージ内容を入力して、<strong>最後に必ず「送信」を押下して下さい</strong>（押下しないと送信されません）。</p>
...
<p><img src="images/message_writescreen.png" class="img-fluid"><br><span class="small text-muted">※実際の画面と異なる場合があります。</span></p>
<p>件名（省略可）とメッセージ内容を入力し、<strong>最後に必ず「送信」を押下して下さい</strong>（押下しないと送信されません）。</p>
...
<p>メッセージ内容を入力し（件名は必要に応じて変更出来ます）、<strong>最後に必ず「送信」を押下して下さい</strong>（押下しないと送信されません）。</p>
<p>「メッセージを送信しました。」と表示されれば、送信完了です。</p>

<h2 id="delete">5. 送信したメッセージを削除する</h2>
<p class="text-danger">送信したメッセージを削除すると、送信者自身だけでなく、宛先のユーザーもそのメッセージを読めなくなります。<strong>一度削除したメッセージは復元出来ませんので、注意して操作して下さい。</strong></p>
